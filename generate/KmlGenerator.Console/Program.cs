...
public static class KmlConsoleProgram
{
    public static async Task<int> RunAsync(string[] args, TextWriter output, TextWriter error)
    {
        ...
        services.AddKmlSuiteHostDiagnostics("kml-generator");
    ...
    public async Task<int> RunAsync(string[] args, TextWriter output, TextWriter error)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(parsed.Value.OutputPath))
            {
                ...
                _logger.LogInformation("Wrote KML output to {OutputPath} with {IntersectionPolygonCount} intersection polygons", parsed.Value.OutputPath, result.IntersectionPolygonCount);
                await output.WriteLineAsync($"Saved {result.IntersectionPolygonCount} intersection polygons to {parsed.Value.OutputPath}");
            }
